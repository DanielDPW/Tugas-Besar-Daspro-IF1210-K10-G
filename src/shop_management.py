    ...
    if utils.is_empty(current_user):
        ...
        while True:
            ...
            if prompt == 'lihat':
                ...
                item_shop_data, item_shop_dict, monster_shop_data, monster_shop_dict = remove(item_shop_data,item_shop_dict,monster_shop_data,monster_shop_dict,monster_data)
            elif prompt == 'keluar':
                utils.clear_terminal()
                return item_shop_data, monster_shop_data
            else:
...
def remove(item_shop_data : Matrix, item_shop_dict : DictList, monster_shop_data : Matrix, monster_shop_dict : DictList, monster_data : Matrix) -> Tuple[Matrix, DictList, Matrix, DictList]:
    while True:
        ...
        if prompt == "item":
            item_shop_data, item_shop_dict = delete('item',item_shop_data,item_shop_dict,monster_data)
            break
        elif prompt == "monster":
            monster_shop_data, monster_shop_dict = delete('monster',monster_shop_data,monster_shop_dict,monster_data)
            break
        else:
            print("Masukkan input yang valid")
            time.sleep(1)
            ...
            if prompt.lower() == 'y':
                ...
                if shop_type == "item":
                    if shop_dict[idx]['type'] == 'strength':
                        item_name = 'Strength Potion'
                    elif shop_dict[idx]['type'] == 'speed':
                        item_name = 'Speed Potion'
                    elif shop_dict[idx]['type'] == 'resilience':
                        item_name = 'Resilience Potion'
                    elif shop_dict[idx]['type'] == 'healing':
                        item_name = 'Healing Potion'
